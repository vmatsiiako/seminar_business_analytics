...
for i in range(len(X_contrast)):
    X_noise_gaussian[i] = add_noise(X_contrast[i, :], noise_type='gaussian')
...
for hidden_dim in HIDDEN_LAYERS:
    ...
    new_data_zeros = np.zeros(np.shape(new_data))
    for i in range(len(new_data)):
        new_data_zeros[i] = add_noise(new_data[i, :], noise_type='zeros', percentage=NOISE_PERCENTAGE)
    # new_data= np.concatenate(new_data, axis=0)
    dae_train_dl_clean = DataLoader(TensorDataset(torch.Tensor(new_data)), batch_size=BATCH_SIZE, shuffle=False)
    dae_train_dl_corrupted = DataLoader(TensorDataset(torch.Tensor(new_data_zeros)), batch_size=BATCH_SIZE, shuffle=False)
